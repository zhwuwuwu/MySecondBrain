...
这一类才真正命中你"同一 agent 功能换模型需要不同 prompt"的核心问题。精读：[[3.1. Cross-Model Prompt Transfer 深度解读]]（4 篇论文对应 4 种完全不同的"跨模型"含义——PromptBridge 是 training-free 文本翻译，MAPO 是 per-model 训练改写器而非真正的迁移，Zero-Shot Continuous Prompt Transfer 是连续向量的跨架构迁移，Transfer-Prompting 实际是跨任务而非跨模型——务必先看这篇再用下表）。
...
多步 agent / tool-use 场景的 prompt 优化与跨模型策略选择。精读：[[3.2. Agent 场景 Prompt 优化深度解读]]（AutoPDL 证明"同一任务不同模型需要不同推理范式"，RePrompt 证明"没有 final checker 时仅靠中间反馈也能做 APE"）。
...
不直接做适配，但支撑"为什么需要适配"的动机与评测方法。精读：[[3.3. Prompt 鲁棒性与评测方法论深度解读]]（含 PromptRobust/PromptBench 的确切关系、Brittlebench 的方差分解公式、POSIX/Multi-Prompt Eval 的具体统计量、AgentBench/AgentBoard 的方法学差异）。
...
- **规模缺口更严重**：本表所有方法验证的都是 <1K token 的短任务指令（PromptBridge 实际优化的 SWE-Bench/Terminal-Bench prompt 也只是 ~800-1100 token 的任务包装层，见 [[3.1. Cross-Model Prompt Transfer 深度解读]] 第1节）；而 OpenCode/Claude Code/Codex CLI 级别的生产 agent system prompt 实测 2,300–22,500 token（角色+工具schema+工作流规则），规模差 5–50 倍，目前没有论文在这个规模上验证过跨模型迁移方法是否还成立。详见新开的 [[9. 长 Agent System Prompt 跨模型可移植性]]。
- 长 system prompt 还有一个平行但不同的问题——**推理速度**（同一份长 prompt 每轮重新 prefill 的延迟成本），这是纯 infra 层问题，和本笔记的"行为一致性"问题正交，整理在 [[5. Agent Prefix 优化]]。
